...
export default function Dashboard() {
  ...
  const refresh = () => {
    api
      .graphql<{ latestSnapshot: AccountSnapshot | null }>(LATEST_SNAPSHOT)
      .then((d) => setSnapshot(d.latestSnapshot))
      .catch(() => setSnapshot(null));
    api
      .graphql<{ trades: Trade[] }>(TRADES, { status: null })
      .then((d) => setTrades(d.trades ?? []))
      .catch(() => setTrades([]));
    api
      .graphql<{ decisions: Decision[] }>(DECISIONS)
      .then((d) => setDecisions(d.decisions ?? []))
      .catch(() => setDecisions([]));
    api
      .graphql<{ riskGateEvents: RiskGateEvent[] }>(RISK_GATE_EVENTS)
  ...
  useEffect(() => {
    refresh();
    return api.subscribe<{ events: string }>("subscription { events }", undefined, (d) => {
    ...
    <div className="mx-auto flex max-w-5xl flex-col gap-6 p-8">
      ...
      <section className="grid grid-cols-2 gap-4 rounded-xl border border-zinc-800 p-4 sm:grid-cols-4">
        ...
        <div>
          ...
          <p
            className={`text-lg font-semibold ${(snapshot?.dayPnl ?? 0) < 0 ? "text-red-400" : "text-emerald-400"}`}
          >
      ...
      <section className="rounded-xl border border-zinc-800 p-4">
        <h2 className="mb-3 text-sm font-medium text-zinc-400">
          Trades ({openTrades.length} open / {trades.length} total)
        </h2>
        {trades.length === 0 ? (
                ...
                {trades.map((t) => (
                  <tr key={t.id} className="border-t border-zinc-900">
                    ...
                    <td className="py-2 pr-4">
                      {t.creditDebit} {money(t.netPremium)}
                    </td>
            ...
            {decisions.map((d) => (
              <li key={d.id} className="border-t border-zinc-900 pt-2 text-sm">
                ...
                <span className="text-xs text-zinc-500">
                  confidence {d.confidence.toFixed(1)} · {d.runDate}
                </span>
